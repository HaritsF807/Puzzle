...
async function initHandLandmarker() {
  ...
  try {
    vision = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Waktu habis saat memuat runtime MediaPipe (WASM). Periksa koneksi internet Anda atau jika cdn.jsdelivr.net diblokir."
    );
  } catch (err) {
    throw err;
  }
  ...
  try {
    const handLandmarker = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Waktu habis saat mengunduh model HandLandmarker (~10MB) dengan GPU."
    );
  ...
  try {
    const handLandmarker = await withTimeout(
      ...
      LOAD_TIMEOUT_MS,
      "Waktu habis saat mengunduh model HandLandmarker (~10MB) bahkan dengan CPU. Periksa koneksi internet Anda atau jika storage.googleapis.com diblokir di jaringan Anda."
    );
    return handLandmarker;
  } catch (cpuErr) {
    throw cpuErr;
...
function drawBoardAndPieces() {
  ...
  if (puzzle.solved) {
    ...
    ctx.textAlign = "center";
    ctx.textBaseline = "middle";
    ctx.fillText("SELESAI! — kepalkan tangan untuk menyimpan", box.x + box.width / 2, box.y + box.height / 2);
    ctx.restore();
  }
}
...
function finishShatter() {
  ...
  if (shatter.pendingCanvas) {
    ...
    statusText.textContent = "tersimpan di strip foto!";
    shatter.pendingCanvas = null;
  }
  resetPuzzleOnly();
}
...
function handleFistReset() {
  if (appState !== "puzzle") {
    statusText.textContent = "direset (kepalan)";
    resetPuzzleOnly();
    return;
  }
...
function processResults(result) {
  ...
  if (noHands) {
    ...
    if (appState === "tracking") {
      ...
      statusText.textContent = isStripFull()
        ? "strip foto lengkap — unduh atau reset"
        : "mencari tangan…";
      return;
    }
    ...
    if (appState === "puzzle") {
      ...
      statusText.textContent = puzzle.solved
        ...
        : "susun puzzle dengan gestur pinch";
      return;
    }
  ...
  if (appState === "tracking") {
    if (isStripFull()) {
      statusText.textContent = "strip foto lengkap — unduh atau reset";
      return;
    }
    if (handsLandmarks.length === 2) {
      ...
      if (bothPinching && frameBox.width > 40 && frameBox.height > 40) {
        ...
        statusText.textContent = "tahan cubitan…";

        if (performance.now() - freezeGate.since > FREEZE_HOLD_MS) {
          freezeGate.holding = false;
          finishCountdownAndCapture(frameBox);
        }
      } else {
        freezeGate.holding = false;
...
async function boot() {
  ...
  const watchdog = setTimeout(() => {
    if (!settled) {
      showLoaderError("Proses memuat terlalu lama. Klik coba lagi atau periksa koneksi internet Anda.");
    }
  }, watchdogMs);
  ...
  try {
    ...
    if (err && err.name === "NotAllowedError") {
      showLoaderError("Izin akses kamera ditolak. Aktifkan di pengaturan browser lalu klik coba lagi.");
    } else if (err && err.name === "NotFoundError") {
      showLoaderError("Tidak menemukan webcam yang tersedia.");
    } else {
      showLoaderError((err && err.message) || "Terjadi kesalahan saat memulai aplikasi.");
    }
  }
}
...
if (resetAllBtn) {
  resetAllBtn.addEventListener("click", () => {
    const confirmed = window.confirm(
      "Apakah Anda yakin ingin menghapus seluruh strip foto dan mulai dari awal?"
    );
